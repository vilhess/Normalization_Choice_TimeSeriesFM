...
class ChronosDataset(Dataset):
    ...
    def __init__(self, file_path: str, normalize: bool = True):
...
class ChronosDataset_mmap(Dataset):
    ...
    def __init__(self, file_path: str, file_shape_path: str, normalize: bool = True):
        if self.EXPECTED_FILENAME not in file_path:
            raise ValueError(f"File path must contain '{self.EXPECTED_FILENAME}'.")
...
def download_chronos_datasets_and_process(
    config: DSPathsConfig = None
) -> None:
    """Download the Chronos datasets from HuggingFace and convert them to numpy arrays."""
    import datasets
    from tqdm import tqdm
    hf_folder = config.chronos_kernel_synth_path.split("/training_corpus_kernel_synth_1m.npz")[0]+"/huggingface"
    np_folder = config.chronos_kernel_synth_path.split("/training_corpus_kernel_synth_1m.npz")[0]
...
if __name__ == "__main__":
    ...
    if DOWNLOAD:
        ...
        download_chronos_datasets_and_process(config=DSPathsConfig())
