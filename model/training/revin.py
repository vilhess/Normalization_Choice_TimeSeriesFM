...
class PrefixRevIN(nn.Module):
    def __init__(self, eps=1e-5, use_asinh=True, prefix_tokens=4):
        super().__init__()
        self.eps = eps
        self.cached_mean = None
        self.cached_std = None
...
class CausalRevIN(nn.Module):
    ...
    def forward(self, x, mode: str):
        ...
        if mode == "norm":
            ...
            if self.use_asinh:
                x64 = torch.sinh(x64)
            out = x64 * self.cached_std.repeat_interleave(dim=-1, repeats=9) + self.cached_mean.repeat_interleave(dim=-1, repeats=9)
    ...
    def _get_statistics(self, x):
        ...
        _, L = x.shape
        counts = torch.arange(1, L+1, device=x.device)
...
class CausalPatchRevIN(nn.Module):
    def __init__(self, eps=1e-5, use_asinh=True):
        super().__init__()
        self.eps = eps
        self.cached_mean = None
